...
import { getNearbyFamilyIds } from "../src/family-view.js";
...
test("exposes history controls and nearby-family view in the interface", async () => {
  const source = await readFile(new URL("../src/App.jsx", import.meta.url), "utf8");
  const styles = await readFile(new URL("../src/styles.css", import.meta.url), "utf8");
  assert.match(source, /Предыдущий/);
  assert.match(source, /Следующий/);
  assert.match(source, /Ближайшая семья/);
...
console.log("Stage 42 navigation and nearby family ok: history, focused relatives, and UI controls");
