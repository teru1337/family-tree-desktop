...
test("keeps a slow entrance and continuous animated background for the start menu", () => {
  assert.match(styles, /--motion-duration-scene: 1500ms/);
  assert.match(styles, /--motion-duration-background-slow: 9s/);
  assert.match(styles, /main-menu-entrance var\(--motion-duration-scene\)/);
  assert.match(styles, /main-menu-ambient var\(--motion-duration-background-slow\) ease-in-out infinite alternate/);
  assert.match(styles, /main-menu-branch-drift 7s ease-in-out infinite alternate/);
  assert.match(styles, /main-menu-halo 1700ms/);
  assert.match(styles, /main-menu-glint var\(--motion-duration-scene\)/);
