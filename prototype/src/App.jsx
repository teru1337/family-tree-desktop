...
import { calculateRelationship, personLabel, relationshipEdgeKey } from "./relationship-calculator.js";
...
const InteractiveFamilySchema = lazy(() => import("./InteractiveFamilySchema.jsx").then(({ InteractiveFamilySchema: Component }) => ({ default: Component })));
...
function TreeNode({ person, position, selected, branchMuted, onSelect, onKeyboardNavigate, showPhotos, showFormerSurnames, cardFields, childNumber, dragging, onDragStart, onDragMove, onDragEnd, collapsible = false, collapsed = false, onToggleCollapse, motionTransform = "", entering = false, exiting = false, pathHighlighted = false, additionRoleName = "", additionPhase = "" }) {
  ...
  return (
    <>
    <button className={`tree-node ${genderClass} ${branchMuted ? "tree-node-branch-muted" : ""} ${selected ? "tree-node-selected" : ""} ${pathHighlighted ? "tree-node-path-highlight" : ""} ${showPhotos ? "" : "tree-node-no-photo"} ${dragging ? "tree-node-dragging" : ""} ${entering ? "tree-node-motion-enter" : ""} ${exiting ? "tree-node-motion-exit" : ""} ${additionRoleName ? `tree-node-addition-${additionRoleName} tree-node-addition-${additionPhase}` : ""}`} data-person-id={person.id} style={{ left: position.left, top: position.top, width: position.width, height: position.height, ...(motionTransform ? { transform: motionTransform } : {}) }} type="button" onClick={() => onSelect(person.id)} onKeyDown={(event) => onKeyboardNavigate?.(person.id, event)} onPointerDown={(event) => onDragStart?.(person.id, event)} onPointerMove={(event) => onDragMove?.(event)} onPointerUp={(event) => onDragEnd?.(event)} onPointerCancel={(event) => onDragEnd?.(event)} aria-pressed={selected} aria-keyshortcuts="ArrowUp ArrowDown ArrowLeft ArrowRight" aria-label={`${cardName}${childNumber ? `, ребёнок номер ${childNumber}` : ""}${cardLines.length ? `, ${cardLines.join(", ")}` : ""}`}>
...
function RelationshipCalculatorModal({ people, partnerships, initialSourceId, onClose, onSelectPerson, onShowOnMap, onHighlightPath }) {
  ...
  return (
    <div className="relationship-calculator-backdrop" role="presentation" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section className="relationship-calculator-card" role="dialog" aria-modal="true" aria-labelledby="relationship-calculator-title" onClick={(event) => event.stopPropagation()}>
        ...
        {result.status === "found" && <div className="relationship-calculator-result"><div className="relationship-result-heading"><div><span className="eyebrow">Результат</span><h3>{result.label}</h3></div><div className="relationship-result-actions"><button type="button" className="button button-secondary" onClick={() => { onShowOnMap(result.target.id); onHighlightPath?.({ label: result.label, personIds: displayPath.map((person) => person.id), edges: result.displayEdges || [] }); }}><Crosshair size={17} /> Показать и подсветить путь</button></div></div><p className="relationship-result-subtitle">Путь от старших к младшим: {Math.max(0, displayPath.length - 2)} промежуточных {displayPath.length - 2 === 1 ? "человек" : "человека"}.</p><div className="relationship-path" aria-label="Путь родства">{displayPath.map((person, index) => <div className="relationship-path-row" key={person.id}><button type="button" className="relationship-path-person" onClick={() => selectPathPerson(person.id)}><PersonAvatar person={person} /><span><strong>{personLabel(person)}</strong><small>{person.year || "дата неизвестна"}</small></span></button>{index < displaySteps.length && <div className="relationship-path-step"><span>{displaySteps[index].label}</span><CaretRight size={15} /></div>}</div>)}</div></div>}
...
function TreeConnections({ people, partnerships, positions, width, height, visibleIds = null, hiddenIds = new Set(), renderIndex = null, strictVisible = false, branchMode = false, branchIds = new Set(), contextIds = new Set(), expandedLabelId = "", onExpandedLabelChange, transitionPositions = null, transitionHiddenIds = null, transitionPhase = "from", additionMotion = null, highlightPersonId = "", highlightEdgeKeys = new Set() }) {
  ...
  const highlightActive = Boolean(highlightPersonId || highlightEdgeKeys.size);
  const edgeHighlightClass = (kind, key, personIds) => {
    if (!highlightActive) return "";
    const pathHighlighted = highlightEdgeKeys.has(key);
    const directlyHighlighted = pathHighlighted || (highlightPersonId && personIds.includes(highlightPersonId));
    if (!directlyHighlighted) return "connection-dimmed";
    return `connection-highlighted connection-highlighted-${kind}${pathHighlighted ? " connection-path-highlight" : ""}`;
  };
  const edgeVisible = (edge, firstId, secondId, edgeHiddenIds = hiddenIds) => !edgeHiddenIds.has(firstId) && !edgeHiddenIds.has(secondId) && (!strictVisible || !visibleIds || (visibleIds.has(firstId) && visibleIds.has(secondId)));
  ...
  const siblingEdges = useMemo(() => highlightActive ? index.siblingEdges.filter(({ first, second }) => positions[first.id] && positions[second.id] && edgeVisible({ first, second }, first.id, second.id)) : [], [index, positions, hiddenIds, strictVisible, visibleIds, highlightActive, highlightPersonId, highlightEdgeKeys]);
  ...
  const labels = useMemo(() => [
    ...parentEdges.map(({ parent, child, type }) => {
      ...
      return {
        ...
        highlight: edgeHighlightClass("parent", relationshipEdgeKey({ kind: "parent", parentId: parent.id, childId: child.id, type }), [parent.id, child.id]),
      };
    }),
    ...partnerEdges.map(({ partnership, first, second }) => {
      ...
      return {
        ...
        highlight: edgeHighlightClass("partnership", relationshipEdgeKey({ kind: "partnership", id: partnership.id, personIds: [first.id, second.id] }), [first.id, second.id]),
      };
    }),
  ], [parentEdges, partnerEdges, positions, branchMode, branchIds, contextIds, highlightPersonId, highlightEdgeKeys]);
  ...
  const additionClass = (edge) => additionMotion && edge.personIds?.length === 2 && edge.personIds.includes(additionMotion.newPersonId) && edge.personIds.includes(additionMotion.targetPersonId) ? `connection-addition connection-addition-${additionMotion.phase}` : "";
  const renderConnectionSvg = (positionMap, className, edgeHiddenIds = hiddenIds) => (
    <svg className={className} width={width} height={height} viewBox={`0 0 ${width} ${height}`} aria-hidden="true">
      <g className="parent-connections">
        {allParentEdges.filter(({ parent, child }) => positionMap[parent.id] && positionMap[child.id] && edgeVisible({ parent, child }, parent.id, child.id, edgeHiddenIds)).map(({ parent, child, type }) => {
          ...
          const highlightClass = edgeHighlightClass("parent", relationshipEdgeKey({ kind: "parent", parentId: parent.id, childId: child.id, type }), [parent.id, child.id]);
          return <path key={`${parent.id}-${child.id}-${type}`} className={`connection-line ${type === "adoptive" ? "connection-adoptive" : ""} ${type === "step" ? "connection-step" : ""} ${edgeMuted(parent.id, child.id) ? "connection-branch-muted" : ""} ${highlightClass} ${extraClass}`} d={geometry.path} />;
        })}
      </g>
      <g className="partnership-connections">
        {allPartnerEdges.filter(({ first, second }) => positionMap[first.id] && positionMap[second.id] && edgeVisible({ first, second }, first.id, second.id, edgeHiddenIds)).map(({ partnership, first, second }) => {
          ...
          const highlightClass = edgeHighlightClass("partnership", relationshipEdgeKey({ kind: "partnership", id: partnership.id, personIds: [first.id, second.id] }), [first.id, second.id]);
          return <path key={partnership.id} className={`connection-line connection-partnership ${partnership.status === "divorced" ? "connection-divorced" : ""} ${edgeMuted(first.id, second.id) ? "connection-branch-muted" : ""} ${highlightClass} ${extraClass}`} d={geometry.path} />;
        })}
      </g>
      <g className="sibling-connections">
        {siblingEdges.filter(({ first, second }) => positionMap[first.id] && positionMap[second.id]).map(({ first, second, type }) => {
          const ordered = positionMap[first.id].left <= positionMap[second.id].left ? [first, second] : [second, first];
          const geometry = horizontalConnection(positionMap[ordered[0].id], positionMap[ordered[1].id]);
          const highlightClass = edgeHighlightClass("sibling", relationshipEdgeKey({ kind: "sibling", personIds: [first.id, second.id], type }), [first.id, second.id]);
          return <path key={`${first.id}-${second.id}-${type}`} className={`connection-line connection-sibling ${highlightClass}`} d={geometry.path} />;
        })}
      </g>
  ...
  return <>{transitionPositions && renderConnectionSvg(transitionPositions, `tree-connections tree-connections-previous ${transitionPhase === "to" ? "tree-connections-previous-to" : ""}`, transitionHiddenIds || hiddenIds)}{renderConnectionSvg(positions, currentClass)}<div className="tree-connection-labels" aria-label="Подписи семейных связей">{positionedLabels.map((label) => { const expanded = expandedLabelId === label.id; return <button key={label.id} type="button" className={`connection-label ${label.muted ? "connection-label-muted" : ""} ${label.highlight || ""} ${expanded ? "expanded" : ""}`} style={{ left: label.left, top: label.top, width: expanded ? label.expandedWidth : label.width, ...(expanded ? { minHeight: label.expandedHeight } : {}) }} aria-expanded={expanded} title={expanded ? "Свернуть полное название связи" : label.full} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); onExpandedLabelChange?.((current) => current === label.id ? "" : label.id); }}>{expanded ? label.full : label.short}</button>; })}</div></>;
}
...
function TreeCanvas({ people, partnerships, layout, selectedId, onSelect, zoom, onZoomChange, pan, onPanChange, treeStyle, showPhotos, showFormerSurnames, cardFields, focusRequest, keyboardPanRequest, inspectorOpen, onToggleInspector, onFocusSelected, viewMode = "full", branchDepth = String(DEFAULT_TREE_BRANCH_DEPTH), branchIds = new Set(), contextIds = new Set(), nearbyIds = new Set(), collapsedIds = new Set(), onToggleCollapse, onResetCollapsedBranches, onViewModeChange, onBranchDepthChange, additionMotion = null, highlightPathIds = new Set(), highlightEdgeKeys = new Set(), highlightStatus = "", onClearHighlight }) {
  ...
  const motionVisiblePeople = includeExitingPeople(visiblePeople, people, exitingIds, renderedPositions);
...
export function App() {
  ...
  const [relationshipHighlight, setRelationshipHighlight] = useState(null);
  ...
  const relationshipHighlightPathIds = useMemo(() => new Set(relationshipHighlight?.personIds || []), [relationshipHighlight]);
  const relationshipHighlightEdgeKeys = useMemo(() => new Set((relationshipHighlight?.edges || []).map(relationshipEdgeKey).filter(Boolean)), [relationshipHighlight]);
  const relationshipHighlightStatus = relationshipHighlight ? `Подсвечен путь родства: ${relationshipHighlight.label || "выбранный путь"}.` : "";
  ...
  const startAdditionMotion = ({ newPersonId, targetPersonId = "", relationKind = "", message }) => {
    ...
    const durations = reduced ? { leadIn: 0, reveal: 0, settle: 0 } : { leadIn: motionDurationMs("120ms"), reveal: motionDurationMs("220ms"), settle: motionDurationMs("180ms") };
    ...
    setAdditionMotion({ ...base, phase: "prepare" });
    setToast("Добавление записи в дерево…");
    additionFrameRef.current = window.requestAnimationFrame(() => {
      setAdditionMotion((current) => current?.token === token ? { ...current, phase: "reveal" } : current);
      additionFrameRef.current = 0;
    });
    additionTimerRef.current = window.setTimeout(() => {
      setAdditionMotion((current) => current?.token === token ? { ...current, phase: "settle" } : current);
      additionTimerRef.current = 0;
  ...
  return (
    <div className={`app-window ${inspectorResizing ? "is-resizing" : ""} ${largeText ? "app-large-text" : ""}`} onClick={() => { if (moreOpen) setMoreOpen(false); }}>
      ...
      <input ref={fileInputRef} className="visually-hidden" type="file" accept=".familytree,.json,application/json" onChange={handleFileSelected} />
       ...
       {relationshipCalculatorOpen && <RelationshipCalculatorModal people={people} partnerships={partnerships} initialSourceId={selectedId} onClose={() => setRelationshipCalculatorOpen(false)} onSelectPerson={selectPerson} onShowOnMap={focusPersonOnMap} onHighlightPath={setRelationshipHighlight} />}
